...
export class ProductBoxFiveComponent implements OnInit {
  ...
  public response: any;
  public backendProducts: any;

  constructor(public productService: ProductService, private http: HttpClient) {
    this.http.get('http://lookc.staging.dynamicweb-cms.com/dwapi/ecommerce/products/search?GroupId=group131').subscribe(res => {
      this.response = res;
      this.backendProducts = this.response.Products[0].Name;
    })
  }
